...
while continueGame
    ...
    switch lower(playerAction)
        ...
        case 'up'
            input('Moving Up...')
            [Map, characterStats] = move(Map, 'up', characterStats, mapLevel, enemyData);

        %Move down
        case 'down'
            input('Moving Down...')
            [Map, characterStats] = move(Map, 'down', characterStats, mapLevel, enemyData);

        %Move left
        case 'left'
            input('Moving Left...')
            [Map, characterStats] = move(Map, 'left', characterStats, mapLevel, enemyData);

        %Move right
        case 'right'
            input('Moving Right...')
            [Map, characterStats] = move(Map, 'right', characterStats, mapLevel, enemyData);
